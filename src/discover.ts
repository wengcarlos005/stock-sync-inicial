      ...
      try {
        ...
        if (item.variations && item.variations.length > 0) {
          for (const v of item.variations) {
            const sku = mac.getMeliVariationSku(v);
            if (!sku) continue;
            const combo = (v.attribute_combinations || []).map(c => c.value_name).filter(Boolean).join('/');
            skuToMeli[sku.trim()] = { item_id: id, variation_id: String(v.id), name: (item.title || '') + (combo ? ' - ' + combo : '') };
          }
        } else {
          const sku = mac.getMeliSku(item);
          if (!sku) continue;
          skuToMeli[sku.trim()] = { item_id: id, variation_id: null, name: item.title || '' };
        }
      } catch (e: any) {
